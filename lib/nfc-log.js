...
module.exports = function (prefix, level) {
  ...
  log.error = function() {
    ...
    // TODO: replace with arrow function when supported
    // txt = args.map(s => printFormat(s));
    var txt = args.map(function(s) { return toPrintableString(s); });
    console.log("[Error] " + prefix + txt);
  };
  ...
  log.warning = function() {
    ...
    var args = Array.prototype.slice.call(arguments, 0);
    var txt = args.map(function(s) { return toPrintableString(s); });
    console.log("[Warning] " + prefix + txt);
  };
