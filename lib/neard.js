...
function setPolling(mode, callback) {
  ...
  // Check the NFC radio Mode: Initiator, Target, or Idle.
  // Actions for (Mode, mode) pairs:
  // - (Idle, none), (Initiator, tag), (Target, peer): return
  // - (Idle, tag), (Idle, peer), (Idle, any): set new mode
  // - (Initiator, none), (Target, none): stop polling
  // - (Initiator, peer), (Target, tag): stop and set new mode
  // - (Initiator, any), (Target, any): stop and set new mode
  // Instead of implementing the matrix above, set either off or Dual polling.
  neard.adapter.getProperty('Polling', function(err, value) {
    if (err) {
      log.error("setPolling: could not get NFC adapter polling state.")
      util.invoke(callback, err);
      return;
    }
    ...
    if (on) {
      ...
      neard.adapter.StartPollLoop('Dual');  // hardcoded always 'Dual'
      return;
    }
...
function onsignal() {
  ...
  neard.manager.on('InterfacesRemoved', function(iface) {
    if (iface == '/org/neard/nfc0') {
      log.debug("NFC adapter removed.");
      neard.adapter = null;
      util.invoke(exports.onadapterdisabled, null);
      return;
    }
    ...
    if (iface.match('\/org\/neard\/nfc0\/device.\/tag.')) {
      ...
      checkPolling(function(err) {
          util.invoke(exports.ondeviceremoved, null);
      });
    }
    ...
    if (iface.match('\/org\/neard\/nfc0\/tag.')) {
      ...
      neard.tag = null;
      checkPolling(function(err) {
          util.invoke(exports.ontagremoved, null);
      });
      return;
    }
...
function getAdapter(callback) {
  ...
  neard.manager.GetManagedObjects['finish'] = function(objects) {
    var obj = objects['/org/neard/nfc0'];
    if (!obj) {
      log.error("getAdapter: could not find default adapter.")
      util.invoke(callback, new Error('NotFoundError'));
      return;
    }
    neard.bus.getInterface('org.neard', '/org/neard/nfc0', 'org.neard.Adapter',
      function(err, iface) {
        if (err) {
          log.error("getAdapter: could not get adapter interface.");
          util.invoke(callback, err);
          return;
        }
        log.debug("NFC adapter found.");
        neard.adapter = iface;
        util.invoke(callback);
      });
  };
...
function enableAdapter(callback) {
  ...
  if (!neard.bus || !neard.manager || !neard.adapter) {
    init(function(error){
      if (error) {
        log.debug("Cannot initialize neard.");
        util.invoke(callback, error);
        return;
      }

      getAdapter(function(err) {
        if (err) {
          log.error("enableAdapter: cannot get NFC adapter.");
          util.invoke(callback, err);
          return;
        }
...
function flush(callback) {
  ...
  if (!(neardMethod instanceof Function) ||
    ...
    util.invoke(neard.outbox.callback, true);
    util.invoke(callback);
    return;
  }
...
function notifyMessage() {
  ...
  util.invoke(neard.onreceive, message);
